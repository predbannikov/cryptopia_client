...
QVariant ModelPairs::data(const QModelIndex &index, int role) const
{
    ...
    if (!index.isValid())
        return QVariant();
    const ModelPairs::Currency &rec = currencies.at(index.row());
    switch(role){
        case Qt::CheckStateRole:
            if (index.column()==filter)
//                return currencies.at(index.row()).filter;
            {
                int id = currencies.at(index.row()).Id;
                if(favoriteCoin.contains(id))
                    return true;
                else
                    return false;
            }
...
//    qDebug() << "chkfiltr = " << chkfiltr;
    if(chkfiltr)
    {
        ...
        {
            if(favoriteCoin.contains(sourceCurrencies[i].Id))
            {
...
void ModelPairs::selectedRow(QModelIndex index)
{
    if(index.column()==0){                  // Разбираемся с чекбоксами фаворитных монет
        int id = currencies[index.row()].Id;
        if(!favoriteCoin.contains(id))      // Если в фаворитных монетках нет,
            favoriteCoin.append(id);        // то добавляем
        else                                // иначе
            favoriteCoin.removeOne(id);     // удаляем из фаворитного списка
